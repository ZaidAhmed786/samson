        ...
        <div key={item._id} className={styles['cart-item-container']}>
          <div className={styles['cart-item']} style={{ display: index === selectedProductIndex ? 'flex' : 'none' }}>
            ...
            <div className={styles.ingredientDiv}>
              ...
              {item?.productId?.ingredients.map((ingredient: Ingredient, idx: number) => {
                const ingredientEntries = Object.entries(ingredient);
                return (
                  <ul key={idx}>
                    <p><b> Ingredients</b></p>
                    <li>
                      {ingredientEntries.slice(0, -1).map(([key, value]) => ( // Exclude the last entry
              ...
              {item?.productId?.extraIngredients?.[0] ? (
                <ul>
                  <p><b>Extra Ingredients</b></p>
                  {item?.productId?.extraIngredients?.[0]?.map((ingredient: any, idx: any) => (
                    <li key={idx}> <b>{ingredient.type}:</b> {ingredient.option}</li>
                  ))}
                </ul>
              ) : null
